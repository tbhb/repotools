...
#
# The review signature is checked here too. The Codex parent records a
# passing independent review through stamp-review.sh; this script proves
# that the reviewed bytes are still the bytes being committed.
#
...
reviewed_message=$(sed -n 's/^message //p' "$stamp")
reviewed_tree=$(sed -n 's/^index //p' "$stamp")
current_tree=$(gitr write-tree)
if [ "$reviewed_message" != "$(digest "$draft")" ] || [ "$reviewed_tree" != "$current_tree" ]; then
  fail "COMMIT_AGENTMSG or the staged tree changed after review-commit-message
signed off, so the reviewed change and the change about to be committed are
no longer the same.

Run review-commit-message again against the current draft, then commit."
fi

# Only --amend passes through. Everything else this workflow needs is
# supplied here. An allowlist prevents a caller from carrying
# --no-verify or another unsafe flag through the wrapper.
amending=false
for arg in "$@"; do
  case $arg in
  --amend) amending=true ;;
...
landed_tree=$(gitr rev-parse 'HEAD^{tree}')
if [ "$landed_tree" != "$reviewed_tree" ]; then
  printf '\nThe commit tree differs from the staged tree the reviewer cleared.\n' >&2
  printf 'A hook changed the index during the commit. Review the resulting tree,\n' >&2
  printf 'then amend it through this workflow rather than treating it as cleared.\n' >&2
  exit 1
fi
...
if [ -n "$missing" ]; then
  ...
  printf '  bash .agents/skills/codex-commit/scripts/commit.sh --amend\n' >&2
  exit 1
fi
