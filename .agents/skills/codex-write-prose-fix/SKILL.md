...
description: >-
  Reword one settled document to clear its prose lint findings without changing meaning or any other file. The codex-fix-prose skill delegates this task with no inherited conversation context.
---
...
Preserve every claim, qualifier, number, path, identifier, and name. Reword, but don't rewrite. Only the target's prose may change. Leave linter configuration, dictionaries, every other file, fenced code, paths, and backticked identifiers untouched. Add no suppression comments. Preserve the target’s wrapping convention.
