...
#
# The codex-review-squash-message skill runs this explicitly. A fresh
# delegated reviewer receives the repository path rather than the caller's
# context, so this command gathers the draft, commits, description, and diff.
#
# Read-only throughout. The merge-pr preflight owns the preconditions
# and the stale-draft report; this gathers material. A reviewer has no
...
if ! view=$(gh pr view "$number" --json number,title,state,url,baseRefName,headRefName,headRefOid \
